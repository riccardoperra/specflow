...
- Plain CSS: Used to override some elements which inside the shadow dom that you don't have access to normally. I
  preferred not to disable shadow dom as the hanko authors do not recommend it

I made a solid component for each web component in order to decouple it, and to extend some behaviors and the JSX
interface, since solid has its own.

First, each web component will have attached a custom class generated by vanilla-extract, then I access to wc shadow dom
after the component mount in order to add some custom styling I need.
...
Each Vanilla Extract class will extend a base class which will define all the custom vars and base styles.
